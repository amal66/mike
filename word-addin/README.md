...
- The Mike API running locally (`npm run dev:api` from the repo root) and Supabase configured per the root [README](../README.md) (`apps/api/.env` + `apps/web/.env.local`)

---

## Quick start (one command)

If the API is already running and `apps/web/.env.local` is filled in, this script does everything below for you — reads the Supabase URL + publishable key, writes `.env.development`, installs dependencies, installs the trusted dev certificate, and launches the add-in into Word:
...
2. **Set environment variables**
   ...
   - `REACT_APP_SUPABASE_URL` / `REACT_APP_SUPABASE_ANON_KEY` — the same values as `apps/web/.env.local`'s `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_DEFAULT_KEY` (from the Supabase dashboard).
...
Confirm the `REACT_APP_SUPABASE_URL` / `REACT_APP_SUPABASE_ANON_KEY` in `.env.development` match `apps/web/.env.local`, and that the URL has no trailing slash.
...
- Confirm the Mike API is running (`npm run dev:api`) and reachable at `http://localhost:3001`
