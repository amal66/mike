...
// Q is deliberately unconstrained: constraining it to the supabase-js builder
// type makes the compiler instantiate the (very deep) chained builder generics
// at every call site (TS2589). The structural cast keeps the runtime contract.
function applyKindFilter<Q>(query: Q, kind: LibraryKind): Q {
  ...
  return (
    kind === "file"
      ? q.or("library_kind.eq.file,library_kind.is.null")
      : q.eq("library_kind", kind)
  ) as Q;
}
