...
export async function createProject(
  ...
  const { userId, userEmail, name, cm_number, practice, shared_with } =
    params;
  if (!name?.trim())
...
export async function updateProject(
  ...
  // Editing a project's name / sharing is a management operation: only the
  // row owner may do it. Email-share collaborators can read but not mutate.
  const access = await checkProjectAccess(projectId, userId, userEmail, db);
  if (!access.ok || !access.isOwner) return { ok: false, kind: "not_found" };
