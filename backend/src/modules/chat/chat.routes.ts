...
chatRouter.post("/", requireAuth, async (req, res) => {
    ...
    // header flush, runLLMStream, abort handling, and assistant-message
    // persistence — stays in this route because its ordering is delicate.
    ...
    // API key fetch must happen BEFORE flushHeaders(). Once SSE headers are
    // flushed the response is committed — we can no longer send an error JSON
    // body.
    ...
    // abort (same path as a disconnect: saves the partial).
