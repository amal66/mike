...
--
-- The overview RPCs re-implement the access logic in SQL for the list views.
-- access.ts now grants a third visibility branch — "row.org_id is in an org the
-- caller belongs to" — alongside "row owner" and "shared_with email". If these
-- RPCs are not updated in lockstep, org-shared rows would be readable via the
-- detail endpoints (which go through access.ts) but invisible in the list
-- views. This migration adds that org-membership branch to the workflows,
-- chats, and projects overviews; the tabular-review read-models (paginated
-- overview + bulk-ids) follow in their own migrations.
--
...
-- keeps meaning "row owner" — org membership grants visibility, not ownership.
--
-- Bodies reproduce the current definitions (post 20260812_01 pagination and
-- 20260813_01 uuid conversion — RPC params stay text, bodies cast at the
-- database boundary) plus the org branch, and match backend/schema.sql exactly.
--
-- create-or-replace only; signatures are unchanged, so no drop is needed.
-- Safe to re-run.
