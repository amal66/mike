...
describe("project assistant chat deletion gating", () => {
    it("lets a project admin delete a colleague's chat", async () => {
        ...
        await renderPage([
            chat("c1", { access_role: "admin", is_owner: false }),
        ]);
    ...
    it("refuses a member their own chat's deletion and never calls the API", async () => {
        ...
        await renderPage([
            chat("c1", { access_role: "member", is_owner: false }),
        ]);
    ...
    it("surfaces a refused delete instead of dropping the row", async () => {
        ...
        await renderPage([chat("c1", { access_role: "admin" })]);
    ...
    it("reports blocked and failed rows after a bulk delete", async () => {
        ...
        await renderPage([
            chat("c-ok", { access_role: "admin" }),
            chat("c-fails", { access_role: "admin" }),
            chat("c-blocked", { access_role: "member" }),
        ]);
        ...
        const notice = await screen.findByText(/skipped because only a project admin/);
        expect(notice.textContent).toContain(
            "1 selected chat was skipped because only a project admin can delete them.",
        );
        expect(notice.textContent).toContain(
