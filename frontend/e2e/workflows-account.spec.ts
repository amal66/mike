...
async function createWorkflowAndOpenDetail(page: Page, title: string) {
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        ...
        // Generous timeout: the local Supabase gateway can keep a create POST in
        // flight ("Creating…") for many seconds under load before it resolves.
        await expect(createBtn).toBeEnabled({ timeout: 30_000 });
        await createBtn.click();
        try {
            // Per-attempt navigation budget covers a slow create POST plus the
            // first on-demand `next dev` compile of the dynamic /workflows/[id]
            // route, so a slow-but-successful create navigates within one attempt.
